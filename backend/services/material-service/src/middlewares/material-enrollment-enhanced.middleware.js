...
/**
 * Enhanced middleware to verify if a user is enrolled in a course for a specific material
 * Includes fallback behavior for service outages
 * Admins bypass this check
 */
export const verifyMaterialEnrollmentEnhanced = async (req, res, next) => {
  try {
    ...
    try {
      ...
      console.error(`Enrollment check failed for user ${userId}, course ${courseId}, material ${materialId}:`, error);
      ...
      if (ENROLLMENT_CHECK_CONFIG.ALLOW_ACCESS_ON_CHECK_FAILURE) {
        console.warn(`Allowing access to material ${materialId} for user ${userId} due to enrollment service failure`);
        
        // Cache this failure so we don't keep trying unsuccessful requests
        const cacheKey = `enrollment-check-failure:${userId}:${courseId}:${materialId}`;
    ...
    console.error('Error in material enrollment verification middleware:', error);
